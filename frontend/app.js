...
function renderTable(data) {
        ...
        elements.grnaTableBody.innerHTML = `
            <tr>
                <td colspan="10" style="text-align:left; padding: 40px; color: var(--text-muted);">
                    <div style="font-size:14px; margin-bottom:8px; font-weight:600;">No Designed Candidates Match Filters</div>
                    <div style="font-size:12px; color:var(--text-inactive);">Try relaxing your selection filters (e.g. min efficiency or risk limits).</div>
                </td>
            </tr>
        `;
        return;
    ...
    data.forEach((grna, idx) => {
        ...
        const starHTML = `<span class="star-toggle" onclick="event.stopPropagation(); window.toggleShortlist('${grna.sequence}')" style="cursor:pointer; font-size:14px; margin-right:8px; color:${isStarred ? '#4a7c59' : 'var(--text-inactive)'};">${isStarred ? '★' : '☆'}</span>`;
...
function renderShortlist() {
    ...
    state.shortlist.forEach((seq, idx) => {
        ...
        tr.innerHTML = `
            ...
            <td><span class="mono seq-align-box" style="padding:2px 4px; font-size:11px;">${grna.spacerSeq}</span></td>
            ...
            <td>
                <button class="quick-q-btn" onclick="window.toggleShortlist('${grna.sequence}')" style="padding:2px 8px; font-size:9px; background:none; border-color:var(--border-neutral); color:var(--text-muted);">
                    Remove
                </button>
            </td>
        `;
...
function openDetailDrawer(seq) {
    ...
    let offTargetsHTML = `<div style="font-size:11.5px; color:var(--text-muted); padding: 4px 0;">No critical off-targets parsed. Perfect specificity profile.</div>`;
    if (grna.off_targets && grna.off_targets.length > 0) {
        offTargetsHTML = `
            <table class="drawer-data-table" style="font-size:12px; margin-top:6px;">
                <thead>
                ...
                <tbody>
                    ${grna.off_targets.map(ot => `
                        <tr style="border-bottom: 1px solid var(--border-neutral);">
                            ...
                            <td style="padding:6px 0; text-transform:uppercase; font-size:10px;">${ot.region}</td>
    ...
    elements.drawerBodyContent.innerHTML = `
        <!-- Candidate Identity -->
        <div>
            <div style="font-size:10px; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.5px;">Candidate Record Profile</div>
        ...
        <div style="background:var(--bg-control); border:1px solid var(--border-neutral); padding:12px; border-radius:2px;">
            <div style="font-size:9.5px; font-weight:700; color:var(--text-muted); text-transform:uppercase; margin-bottom:4px;">Spacer (5' → 3') + PAM</div>
            <div class="mono" style="font-size:14px; font-weight:700; letter-spacing:0.5px; color: var(--text-main);">
        ...
        <div style="background:var(--primary-bg); border:1px solid ${recColor}; border-radius:2px; padding:12px;">
            <div style="font-size:12px; font-weight:700; color:${recColor}; text-transform: uppercase; letter-spacing: 0.3px;">${recStatus}</div>
            <div style="font-size:12px; line-height:1.45; color:var(--text-main); margin-top:6px;">
                ${recExplanation}
            </div>
        </div>
        ...
        <div>
            ...
            <table class="drawer-data-table">
                <tr><td class="lbl">Predicted Efficiency</td><td class="val" style="color:var(--primary); font-size:13px;">${grna.score.toFixed(4)}</td></tr>
        ...
        <div style="margin-top:20px; display:flex; gap:10px;">
            <button class="btn" style="flex:1; padding:8px 12px; font-size:12px; border-radius: 2px;" onclick="window.toggleShortlist('${grna.sequence}')">
                ${state.shortlist.includes(grna.sequence) ? 'Unstar Candidate' : 'Star Candidate'}
            </button>
            <button class="btn btn-secondary" style="width:60px; padding:0; border-radius: 2px; font-size: 11px;" onclick="window.copyToClipboard('${grna.sequence}', 'drawer-copy')" id="drawer-copy" title="Copy spacer+PAM">
                COPY
            </button>
        </div>
    `;
...
function buildCriticalGenesExplorer() {
    ...
    if (allCriticalGenes.length === 0) {
        container.innerHTML = `
            <div style="text-align:left; padding: 24px; color: var(--text-muted); font-size: 13px;">
    ...
    allCriticalGenes.forEach(gene => {
        ...
        row.innerHTML = `
            <div class="gene-explorer-trigger">
                ...
                <span class="arrow" style="font-size:12px; color:var(--text-muted);">Expand deep-dive papers</span>
            </div>
            <div class="gene-papers-content">
                <div class="papers-loader" style="text-align:left; padding: 20px; color: var(--text-muted); font-size: 12px;">
...
async function fetchGeneLiterature(gene, gridEl, loaderEl) {
    ...
    try {
        ...
        if (papers.length === 0) {
            gridEl.innerHTML = `<div style="font-size:12px; color:var(--text-muted); text-align:left;">No direct papers resolved for '${gene}' in OpenAlex DB. Try broader search.</div>`;
            return;
        }
        ...
        papers.forEach(paper => {
            ...
            card.innerHTML = `
                <div style="font-weight:600; font-size:13px; margin-bottom:4px; color:var(--text-main);">${paper.title}</div>
        ...
        loaderEl.style.display = 'none';
        gridEl.innerHTML = `<div style="font-size:12px; color:var(--error); text-align:left;">Failed to load OpenAlex papers: ${e.message}</div>`;
    }
}
...
async function searchLiterature(query) {
    elements.papersListContainer.innerHTML = `
        <div style="text-align:left; padding: 40px; color: var(--text-muted); font-size: 13px;">
    ...
    try {
        ...
        elements.papersListContainer.innerHTML = `
            <div style="text-align:left; padding: 40px; color: var(--error); font-size: 13px;">
